...
export default function QuizPreview() {
  ...
  // === NEW: which question is currently shown ===
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        const data = await getQuizDetails(qid);
        setQuiz(data);
        setIdx(0); // NEW: start from the first question when quiz loads
      } catch (e: any) {
        setError(
  ...
  const results = useMemo(() => {
    ...
    const details = questions.map((q) => {
      ...
      if (q.type === "MCQ") correct = Number(a) === Number(q.correctIndex);
        ...
        correct = Boolean(a) === Boolean(q.correctTrue);
      else if (q.type === "FIB") {
        const set = (q.correctAnswers ?? []).map(norm);
        correct = set.length > 0 && set.includes(norm(a));
      }
  ...
  return (
    <div id="wd-quiz-preview" className="p-3">
      ...
      {submitted && results && (
        <Alert variant="info">
          Preview score: <b>{results.score}</b> / {totalPoints}
        </Alert>
      )}
      ...
      <Form onSubmit={submit}>
        ...
        <div className="d-flex flex-column gap-3">
          {questions.map((q: any, i: number) => {
            // === NEW: render only the current question ===
            ...
            if (submitted) {
              if (q.type === "MCQ")
                correct = Number(a) === Number(q.correctIndex);
              else if (q.type === "TRUE_FALSE")
                correct = Boolean(a) === Boolean(q.correctTrue);
              else if (q.type === "FIB") {
                const set = (q.correctAnswers ?? []).map(norm);
        ...
        {/* NEW: Next button placed to the LEFT of Submit */}
        <div className="d-flex justify-content-end gap-2 mt-3">
          {/* NEW: Previous button */}
          <Button
          ...
          {!submitted ? (
            ...
            <Button
              ...
              onClick={() => {
                setSubmitted(false);
                setAnswers({});
                setIdx(0); // restart at first question
              }}
            >
              Retake (Preview)
            </Button>
